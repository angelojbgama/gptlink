...
def register(server: MCPServer, operations: GatewayOperations, caller: str) -> None:
    ...
    async def devices_list() -> list[DeviceInfo]:
        ...
        return await operations.list_devices(caller=caller)
    ...
    async def device_info(device_id: UUID) -> DeviceInfo:
        ...
        return await operations.device_info(device_id=device_id, caller=caller)
    ...
    async def lease_acquire(device_id: UUID, ttl_seconds: float = 60) -> LeaseInfo:
        ...
        return await operations.acquire_lease(device_id=device_id, caller=caller, ttl=ttl_seconds)
    ...
    async def lease_release(device_id: UUID, lease_id: UUID) -> ReleaseResult:
        ...
        return await operations.release_lease(device_id=device_id, lease_id=lease_id, caller=caller)
